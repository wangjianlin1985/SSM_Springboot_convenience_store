	...
	<div class="col-md-9 wow fadeInLeft">
		...
		<div class="row">
			...
			<div class="col-md-3 bottom15" <%=clearLeft %>>
			  <a  href="<%=basePath  %>Product/<%=product.getProductId() %>/frontshow"><img class="img-responsive" src="<%=basePath%><%=product.getMainImage()%>" /></a>
			     <div class="showFields">
			     	<div class="field">
	            		商品id:<%=product.getProductId() %>
			     	</div>
			     	<div class="field">
	            		商品类别:<%=product.getProductClassObj().getProdcutClassName() %>
			     	</div>
			     	<div class="field">
                ...
                <select id="productClassObj_productClassId" name="productClassObj.productClassId" class="form-control">
                	<option value="0">不限制</option>
 					...
 					if(productClassObj!=null && productClassObj.getProductClassId()!=null && productClassObj.getProductClassId().intValue()==productClassTemp.getProductClassId().intValue())
 						selected = "selected";
	 				%>
 				 <option value="<%=productClassTemp.getProductClassId() %>" <%=selected %>><%=productClassTemp.getProdcutClassName() %></option>
	 				<%
	 				}
	 				%>
 			</select>
  ...
  <div class="modal-dialog" style="width:900px;" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
		  	 <label for="product_mainImage_edit" class="col-md-3 text-right">商品主图:</label>
		  	 <div class="col-md-9">
			    <img  class="img-responsive" id="product_mainImageImg" border="0px"/><br/>
			    <input type="hidden" id="product_mainImage" name="product.mainImage"/>
			    <input id="mainImageFile" name="mainImageFile" type="file" size="50" />
		  	 </div>
		  </div>
		  <div class="form-group">
	...
	$.ajax({
		...
		success : function (product, response, status) {
			if (product) {
				...
				$.ajax({
					...
					success: function(productClasss,response,status) { 
		        		...
		        		$(productClasss).each(function(i,productClass){
		        			html += "<option value='" + productClass.productClassId + "'>" + productClass.prodcutClassName + "</option>";
		        		});
				...
				$("#product_mainImageImg").attr("src", basePath +　product.mainImage);
