...
function initManagerManageTool() {
	manager_manage_tool = {
		init: function() {
			$.ajax({
				...
				success : function (data, response, status) {
					...
					$("#shopObj_shopId_query").combobox("loadData",data); 
				}
			});
		},
		reload : function () {
		...
		search: function() {
			...
			queryParams["managerUserName"] = $("#managerUserName").val();
			queryParams["shopObj.shopId"] = $("#shopObj_shopId_query").combobox("getValue");
				...
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						for (var i = 0; i < rows.length; i ++) {
							managerUserNames.push(rows[i].managerUserName);
						}
						$.ajax({
							type : "POST",
							url : "Manager/deletes",
							data : {
								managerUserNames : managerUserNames.join(","),
							},
							beforeSend : function () {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "Manager/" + rows[0].managerUserName +  "/update",
					type : "get",
					data : {
						//managerUserName : rows[0].managerUserName,
					},
					beforeSend : function () {
						$.messager.progress({
							text : "正在获取中...",
