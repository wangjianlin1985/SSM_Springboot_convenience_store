$(function () {
	$("#manager_managerUserName").validatebox({
		required : true, 
	...
	$("#manager_shopObj_shopId").combobox({
	    url:'Shop/listAll',
	    valueField: "shopId",
	    textField: "shopName",
	    panelHeight: "auto",
        ...
        onLoadSuccess: function () { //数据加载完毕事件
            var data = $("#manager_shopObj_shopId").combobox("getData"); 
            if (data.length > 0) {
                $("#manager_shopObj_shopId").combobox("select", data[0].shopId);
            }
        }
	});
